    ...
    try {
      ...
      if (quantity < 2 && firstAdd) {
        Swal.fire({
          ...
          imageUrl: `https://fake-comb.herokuapp.com${item?.image}`,
          imageWidth: 400,
          imageHeight: 200,
...
export const requestCart = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await axios.get("https://fake-comb.herokuapp.com/cart", {
        headers: {
          Authorization: bearerToken(),
        },
      });
  ...
  return async (dispatch, getState) => {
    ...
    try {
      const { data } = await axios.get(
        `https://fake-comb.herokuapp.com/order/checkout`,
        {
          headers: {
            authorization: bearerToken(),
          },
      ...
      Swal.fire("Good job!", `Order CheckedOut successfully`, "success");
